...
class GitHubAnalytics:
    ...
    def _get_github_app_token(self):
        ...
        if response.status_code == 201:
            ...
            raise Exception(
                f"Failed to obtain access token: {response_data.get('message', 'Unknown error')}"
            )
    ...
    def analyze_contributions(
        ...
        prompt = ChatPromptTemplate.from_template(
            """
        Analyze the following GitHub contribution summary data and provide a detailed summary:
    ...
    def analyze_commit_messages(self, repo_name: str, commits: List[Dict]) -> str:
        ...
        return chain.run({"commit_messages": commit_messages})

    def get_user_code_patches(
        self,
        repo_name: str,
        username: str,
        start_date: Optional[datetime] = None,
        ...
        for commit in repo.get_commits(author=username):
            ...
            if response.status_code != 200:
                raise Exception(
                    f"Failed to fetch commit details: {response.json().get('message', 'Unknown error')}"
                )
    ...
    def analyze_large_code_patches(self, code_patches: List[Dict]) -> str:
        ...
        formatted_patches = "\n".join(
            [
                f"- {patch['date'].strftime('%Y-%m-%d')}: {patch['message']}\n{patch['patch']}"
                for patch in code_patches
            ]
        )

        return chain.run({"code_patches": formatted_patches})
    ...
    def analyze_user_contributions(
        ...
        if response.status_code != 200:
            raise Exception(
                f"Failed to get repositories: {response.json().get('message', 'Unknown error')}"
            )
        ...
        for repo in repos_data:
            ...
            contributions = self.get_repository_contributors(
                repo_name, start_date, end_date
            )
            user_contributions = next(
                (c for c in contributions if c["login"] == username), None
            )

            if user_contributions:
                # Get user code patches
                code_patches = self.get_user_code_patches(
                    repo_name, username, start_date, end_date
                )
                ...
                """,
                    (
                        repo_name,
                        username,
                        user_contributions["total_commits"],
                        user_contributions["lines_added"],
                        user_contributions["lines_deleted"],
                ...
                if code_patches:
                    ...
                    print(
                        f"Large Code Patches Analysis for {username} in {repo_name}: "
                    )
                    print(large_code_analysis)

                user_contributions_data[repo_name] = {
                    "contributions": user_contributions,
                    "analysis": analysis,
                    "large_code_analysis": large_code_analysis,
                }
        ...
        return user_contributions_data

    def get_users_repositories(self) -> Dict[str, List[str]]:
        """
        Get a dictionary of users and their associated repositories.
        """
        # Use the GitHub API to get repositories accessible by the app
        installation_repos_url = "https://api.github.com/installation/repositories"
        headers = {
        ...
        # Create a dictionary to hold users and their repositories
        user_repos = {}

        for repo in repos_data:
            # Get the owner (user) of the repository
            owner = repo["owner"]["login"]
            repo_name = repo["full_name"]

            if owner not in user_repos:
                user_repos[owner] = []

            user_repos[owner].append(repo_name)
